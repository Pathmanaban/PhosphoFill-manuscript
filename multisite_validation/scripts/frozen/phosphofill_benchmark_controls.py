...
def archive_case_outputs(archive_case_dir: str, row: Dict[str, Any], run_meta: Dict[str, Any], chain_id: str,
    ...
    for pos in target_positions:
        site_key = f'{chain_id}:{pos}'
        # A benchmark row may list a site whose experimental phosphate atoms
        # are absent.  ref_phosphates then contains the position with a None
        # value; normalize that value to an empty mapping for archive creation.
        details = prescan_stage_details(prescan_frames.get(site_key, {}), ref_phosphates.get(pos) or {})
...
def _preferred_output_columns() -> List[str]:
    return [
        "tier", "benchmark_row_index", "benchmark_case_key", "multi_site_case_id", "primary_type",
        "control_mode", "run_graft_mode", "graft_mode", "run_site_order", "run_minimization_coupling",
...
def main():
    ...
    ap.add_argument("--control-mode", required=True, choices=["n_to_c", "c_to_n", "independent", "joint_min"], help="Named multi-site sensitivity control")
    ...
    observed_control = (args.graft_mode, args.site_order, args.minimization_coupling)
    if observed_control != expected_controls[args.control_mode]:
        raise ValueError(
            f"Control {args.control_mode} requires graft_mode/site_order/minimization_coupling="
            f"{expected_controls[args.control_mode]}, observed {observed_control}"
        )
    ...
    if args.resume and os.path.exists(results_path):
        ...
        error_case_keys = {
            ...
            if str(result.get("status", "")).strip().upper() == "ERROR"
            and str(result.get("benchmark_case_key", "")).strip()
        }
        if error_case_keys:
            all_results = [
                result for result in all_results
                if str(result.get("benchmark_case_key", "")).strip() not in error_case_keys
            ]
            print(f"Retrying {len(error_case_keys)} checkpointed case(s) with status=ERROR")
        print(f"Loaded {len(all_results)} retained checkpointed result rows from {results_path}")
    completed_case_keys = {
        str(result.get("benchmark_case_key", "")).strip()
        for result in all_results
    ...
    try:
        for weight in weights:
            for pack_weight in pack_weights:
                ...
                extra_args.extend(["--site-order", args.site_order])
                extra_args.extend(["--minimization-coupling", args.minimization_coupling])

                print("\n" + "#" * 70)
                print(f"Running multi-site control={args.control_mode}, graft_mode={args.graft_mode}, site_order={args.site_order}, minimization_coupling={args.minimization_coupling}, prescan_contact_weight={weight_label}, prescan_pack_weight={pack_label}")
                print("#" * 70)
                archive_run_tag = sanitize_label(f"control-{args.control_mode}__mode-{args.graft_mode}__order-{args.site_order}__min-{args.minimization_coupling}__w-{weight_label}__pack-{pack_label}")

                for i, row in enumerate(rows):
                    acc = row.get("ACC_ID", "?")
                    pdb1 = row.get("PDBID_1", "?")
                    case_key = f"row-{i + 1}__control-{args.control_mode}__mode-{args.graft_mode}__order-{args.site_order}__min-{args.minimization_coupling}__w-{weight_label}__pack-{pack_label}"
                    ...
                    try:
                        ...
                        for r in results:
                            ...
                            r["multi_site_case_id"] = row.get("multi_site_case_id", "")
                            r["primary_type"] = row.get("primary_type", "")
                            r["control_mode"] = args.control_mode
                            r["run_graft_mode"] = args.graft_mode
                            r["run_site_order"] = args.site_order
                            r["run_minimization_coupling"] = args.minimization_coupling
                        ...
                        all_results.append({"tier": 1, "pdb_id": pdb1, "status": "ERROR",
                                            ...
                                            "prescan_pack_weight": pack_weight if pack_weight is not None else "",
                                            "multi_site_case_id": row.get("multi_site_case_id", ""),
                                            "primary_type": row.get("primary_type", ""),
                                            "control_mode": args.control_mode,
                                            "run_graft_mode": args.graft_mode,
                                            "run_site_order": args.site_order,
                                            "run_minimization_coupling": args.minimization_coupling,
