...
def main():
    ...
    ap.add_argument("--environment-contrast", choices=["salt_vs_none", "salt_vs_all_other"],
                    default="salt_vs_none")
    ap.add_argument("--output-basename", default="fig6_statistical_panels_environment_corrected")
    args = ap.parse_args()
    os.makedirs(args.outdir, exist_ok=True)
    plt.rcParams.update(RC)

    single_full = load_single(args.phosphofill_tsv)
    ...
    common = single_full[KEYS].drop_duplicates()
    for df in [pytms_default_ok, pytms_ok, ptmpsi_ok]:
        common = common.merge(df[KEYS].drop_duplicates(), on=KEYS, how="inner")
    if len(common) != 785:
        raise ValueError(f"Expected 785 common comparator sites; found {len(common)}")
    single = single_full.merge(common, on=KEYS, how="inner")
    ...
    if args.audit_tsv:
        ...
        for af in args.audit_tsv:
            ...
            expected = next((res for res in ["TPO","SEP","PTR"] if res in Path(af).stem.upper()), None)
            if expected is None or "actual_resname" not in adf.columns:
                raise ValueError(f"Cannot verify phosphoresidue identity in {af}")
            adf = adf.loc[adf["status"].eq("ok") & adf["actual_resname"].eq(expected)].copy()
        ...
        if audit.duplicated(KEYS).any():
            raise ValueError("Environment audit contains duplicate exact structure-level keys")
        environment_source = (single_full if args.environment_contrast == "salt_vs_all_other"
                              else single)
        merged_audit = environment_source.merge(audit[KEYS + ["interaction_class"]], on=KEYS,
                                    how="inner", validate="one_to_one")
        expected_matches = 775 if args.environment_contrast == "salt_vs_all_other" else 773
        if len(merged_audit) != expected_matches:
            raise ValueError(f"Expected {expected_matches} structure-specific environment matches; found {len(merged_audit)}")
    ...
    ax_f.set_xlabel("S2-S1 RMSD gap (Å)"); ax_f.set_ylabel("Final RMSD (Å)")
    ...
    if len(merged_audit)>0:
        ...
        for rt in ["TPO","SEP","PTR"]:
            ...
            if args.environment_contrast == "salt_vs_all_other":
                group_masks = [
                    (sub["interaction_class"].eq("salt_bridge_like"), "Salt-bridge-like"),
                    (sub["interaction_class"].isin(["hbond_like", "water_mediated_like", "none_obvious"]),
                     "Other classes"),
                ]
            else:
                group_masks = [
                    (sub["interaction_class"].eq("salt_bridge_like"), "Salt-bridge-like"),
                    (sub["interaction_class"].eq("none_obvious"), "No obvious contact"),
                ]
            for mask, lab in group_masks:
                g = sub[mask]
    ...
    for ext in ["png","pdf"]:
        fig.savefig(f"{args.outdir}/{args.output_basename}.{ext}",dpi=300,bbox_inches="tight")
    plt.close(fig)
    print(f"Figure 6 saved from {len(common)} common sites; environment matches: {len(merged_audit)}.")
    print(f"Panel f: Spearman rho={rho:.8g}, p={rho_p:.8g}, n={int(valid.sum())}.")
